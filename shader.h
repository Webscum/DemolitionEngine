...
void freeImg(Shader* s) {
	GPU_FreeImage(s->img);
}

void addImg(Shader* s, char* path) {
	s->img = GPU_LoadImage(path);

	GPU_SetSnapMode(s->img, GPU_SNAP_NONE);
	GPU_SetWrapMode(s->img, GPU_WRAP_REPEAT, GPU_WRAP_REPEAT);
}

void addVariable(Shader* s, int index) {
	Uint32 location = GPU_GetUniformLocation(s->p, idV);
	s->variables.push_back(std::make_pair(idV, location));
}

void initShader(Shader* shd, path){
